...
public final class VisionConstants {
  ...
  // Z error tolerance for accepted poses. Your practice field has ~4-inch Z
  // offset from tag mounting height or camera transform calibration — 6 inches
  // accommodates that while still rejecting obvious flips (which produce Z
  // errors of 0.5m+). Tighten to 1.5 inches once camera transforms are
  // recalibrated and practice field tags are remeasured.
  public static double MAX_Z_ERROR = Units.inchesToMeters(6);
   ...
   * single-tag because multiple coplanar tags improve translational accuracy
   * even when rotational ambiguity persists. Rotation is already distrusted
   * (angular stddev = MAX_VALUE), and the inter-frame jump check is the
   * primary safety net against actual flips.
   */
  public static final double COPLANAR_MAX_DISTANCE_METERS = 7.0;

  /**
   * Maximum distance for true single-tag observations. Beyond this the PnP
   * solver cannot reliably distinguish the true pose from the 180°-flipped
   * alternate.
   */
   ...
   * because a single tag at medium range is the highest flip risk.
   */
   ...
   * threshold as general multi-tag — the distance gate and inter-frame jump
   * check provide the primary flip protection, not the ambiguity gate.
   */
  ...
  /**
   * Maximum physical robot speed in m/s. Sourced from TunerConstants to
   * stay in sync with the actual drivetrain configuration.
   */
  ...
  /**
   * Multiplier on top of max speed to account for encoder noise, wheel
   * slip, and brief speed spikes during collisions.
   */
  public static final double SPEED_TOLERANCE_MULTIPLIER = 1.5;

  /**
   * Minimum displacement tolerance (meters) applied regardless of dt.
   * Handles vision jitter, small corrections, and prevents false positives
   * on the first frames after a detection gap.
   */
  public static final double BASE_JUMP_TOLERANCE_METERS = 0.5;

  /**
   * Maximum dt (seconds) used for flip detection. If the gap between
   * observations exceeds this, dt is clamped to prevent unbounded
   * tolerance. At 3s: max = 3*4.76*1.5+0.5 = 21.9m — well above any
  ...
  public static double MAX_POSE_DISCREPANCY_METERS = Units.inchesToMeters(12);
  ...
  public static double POSE_RESEED_THRESHOLD_METERS = Units.inchesToMeters(24);
