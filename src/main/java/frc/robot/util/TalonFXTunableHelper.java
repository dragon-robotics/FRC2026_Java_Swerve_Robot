...
public class TalonFXTunableHelper {
  ...
  public static final String[] CONTROL_STRATEGIES = new String[] {
      "PositionVoltage",
  ...
  public TalonFXTunableHelper(TalonFX motor, String baseKey, TalonFXConfiguration config, String motorName) {
    ...
    statorCurrentLimit = DogLog.tunable(baseKey + "StatorCurrentLimit", config.CurrentLimits.StatorCurrentLimit,
        newLimit -> {
          motor.getConfigurator()
              .apply(config.withCurrentLimits(config.CurrentLimits.withStatorCurrentLimit(newLimit)));
        });
    ...
    targetPositionSetpoint = DogLog.tunable(baseKey + "PositionSetpoint", 0.0, newPositionSetpoint -> {
      motor.setControl(motorMotionMagicExpoTorqueCurrentFOCRequest.withPosition(newPositionSetpoint));
    });

    targetVelocitySetpoint = DogLog.tunable(baseKey + "VelocitySetpoint", 0.0, newVelocitySetpoint -> {
    ...
    String defaultControlType = motorName.contains("Roller") ? "VelocityTorqueCurrentFOC"
        : "MotionMagicExpoTorqueCurrentFOC";
