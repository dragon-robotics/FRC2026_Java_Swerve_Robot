...
public class ClimberSubsystemSim extends ClimberSubsystem {
  ...
  public ClimberSubsystemSim(ClimberIO climberMotorIO) {
    ...
    climberMech =
        root.append(
            new MechanismLigament2d(
                "Climber",
                climberLength * visualScaleFactor,
                90,
                ARM_WIDTH,
    ...
    climberSim =
        new SingleJointedArmSim(
            motorType,
            CLIMBER_GEAR_RATIO,
            SingleJointedArmSim.estimateMOI(CLIMBER_LENGTH_METERS, CLIMBER_MASS_KG),
  ...
  public void simulationPeriodic() {
    ...
    double motorPosition = Radians.of(climberSim.getAngleRads() * CLIMBER_GEAR_RATIO).in(Rotations);
  ...
  public void periodic() {
    ...
    DogLog.log("Climber/Hook Angle (deg)", String.valueOf(Units.radiansToDegrees(currentAngleRad)));

    DogLog.log(
        "Climber/Hook Velocity (deg/s)",
        String.valueOf(Units.radiansToDegrees(climberSim.getVelocityRadPerSec())));

    DogLog.log("Climber/Hook Current (A)", String.valueOf(climberSim.getCurrentDrawAmps()));
